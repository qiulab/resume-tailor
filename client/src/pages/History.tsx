...
function ScoreBadge({ score }: { score: number | null }) {
  ...
  return (
    <Badge variant="secondary" className={`text-xs font-semibold ${cls}`}>
      {rounded}
    </Badge>
  );
}
...
export default function History() {
  ...
  const deleteHistory = trpc.resume.deleteHistory.useMutation({
    onSuccess: () => {
      utils.resume.getHistory.invalidate();
      ...
      <div className="container py-10 max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Your past analyses and optimizations — pick up where you left off.
          </p>
        </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {analysisToDelete?.companyName ? ` at ${analysisToDelete.companyName}` : ""}, including all
              suggestions and the generated cover letter. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => deleteId && deleteHistory.mutate({ analysisId: deleteId })}
