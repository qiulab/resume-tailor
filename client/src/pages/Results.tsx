...
function ATSRing({ score }: { score: number }) {
  ...
  const color =
    score >= 75 ? "oklch(0.62 0.15 150)" : score >= 50 ? "oklch(0.72 0.12 75)" : "oklch(0.58 0.2 25)";
...
// ─── Polling hook ──────────────────────────────────────────────────────────
function usePollingStatus(analysisId: number, onComplete: () => void) {
...
export default function Results() {
  ...
  const { data, isLoading: dataLoading, refetch } = trpc.resume.getAnalysis.useQuery(
    { analysisId },
    { enabled: isReady }
  );

  const updateSuggestion = trpc.resume.updateSuggestion.useMutation({
    onSuccess: () => utils.resume.getAnalysis.invalidate({ analysisId }),
  });

  const regenerateCoverLetter = trpc.resume.regenerateCoverLetter.useMutation({
    onSuccess: () => {
      utils.resume.getAnalysis.invalidate({ analysisId });
  ...
  const handleSuggestionAction = (id: number, action: "accepted" | "rejected") => {
    updateSuggestion.mutate({ suggestionId: id, status: action });
  };
        ...
        <div className="text-center max-w-sm mx-auto px-6">
          {status === "failed" ? (
            ...
            <>
              ...
              <p className="text-muted-foreground text-sm mb-6">{processingMsg}</p>
              <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
                <Clock className="w-3.5 h-3.5" />
                <span>Usually takes 30–60 seconds</span>
              </div>
              <Progress className="mt-4 h-1" value={undefined} />
            </>
          )}
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-8">
        ...
        <div className="bg-card border border-border rounded-2xl p-8 mb-6">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-8">
            ...
            <div className="shrink-0">
              ...
              <p className="text-xs text-center text-muted-foreground mt-2">ATS Compatibility</p>
            </div>

            {/* Breakdown bars */}
            <div className="flex-1 space-y-4 w-full">
              <h2 className="font-serif font-semibold text-xl text-foreground mb-4">
                ATS Compatibility Score
              </h2>
              {[
                { label: "Keyword Match", value: breakdown.keywords ?? 0, color: "bg-blue-500" },
                { label: "Skills Alignment", value: breakdown.skills ?? 0, color: "bg-emerald-500" },
                { label: "Experience Relevance", value: breakdown.experience ?? 0, color: "bg-amber-500" },
                { label: "Format & Structure", value: breakdown.format ?? 0, color: "bg-purple-500" },
              ].map((item) => (
                <div key={item.label}>
            ...
            <div className="shrink-0 grid grid-cols-2 gap-3 md:grid-cols-1">
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-center">
                <div className="text-2xl font-serif font-bold text-blue-600">{acceptedSuggestions.length}</div>
                <div className="text-xs text-blue-700 mt-0.5">Accepted</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-6 h-11 bg-secondary/60 p-1 rounded-xl">
            <TabsTrigger value="suggestions" className="gap-2 rounded-lg text-sm">
              <PenTool className="w-3.5 h-3.5" />
              Suggestions
              {pendingSuggestions.length > 0 && (
                <Badge variant="secondary" className="ml-1 h-4 w-4 p-0 flex items-center justify-center text-[10px] bg-amber-100 text-amber-700">
            ...
            <div className="space-y-3">
              {suggestions.length === 0 ? (
                <div className="bg-card border border-border rounded-2xl p-12 text-center">
                  ...
                  <p className="font-medium text-foreground">Your resume looks great!</p>
                  <p className="text-sm text-muted-foreground mt-1">No major suggestions were generated.</p>
                </div>
              ) : (
                    ...
                    <div className="p-5">
                      ...
                      {suggestion.status !== "pending" && (
                        <Button
                          ...
                          onClick={() => handleSuggestionAction(suggestion.id, "pending" as any)}
                          disabled={updateSuggestion.isPending}
                        >
                ...
                {missingKeywords.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {missingKeywords.map((kw) => (
                      <span
                        key={kw}
                        className="text-xs px-3 py-1.5 bg-red-50 text-red-700 border border-red-100 rounded-full font-medium"
                      >
                        {kw}
                      </span>
                    ))}
                ...
                {matchedKeywords.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {matchedKeywords.map((kw) => (
                      <span
                        key={kw}
                        className="text-xs px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full font-medium"
                      >
              ...
              <div className="bg-card border border-emerald-200 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => regenerateSummary.mutate({ analysisId })}
          ...
          <TabsContent value="coverletter">
            <div className="bg-card border border-border rounded-2xl p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => regenerateCoverLetter.mutate({ analysisId, tone: coverTone })}
              ...
              {regenerateCoverLetter.isPending ? (
                ...
                <div className="prose prose-sm max-w-none bg-secondary/30 rounded-xl p-6 border border-border text-foreground leading-relaxed whitespace-pre-wrap text-sm">
                  {coverLetter.content}
                </div>
              ) : (
